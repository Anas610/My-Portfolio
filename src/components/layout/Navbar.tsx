...
export const Navbar = ({ enableScrollSpy = true }: NavbarProps) => {
  ...
  return (
    <nav className="fixed z-100 w-full bg-(--color-nav)">
      <div className="mx-auto max-sm:w-[95%] sm:w-[90%]">
        <div className="flex flex-wrap items-center justify-between py-[1.3rem] text-[1.2rem] capitalize text-white max-sm:py-4">
          <Link
            to="/"
            className="text-(--primary-color) font-semibold text-2xl uppercase tracking-tight"
          ...
          <ul
            className={`list-none transition-all duration-300 md:flex md:w-1/2 md:flex-row md:flex-nowrap md:items-center md:justify-between md:p-0 lg:w-1/2 ${menuOpen
              ...
              return (
                <li key={link.id}>
                  <HashLink
                    ...
                    to={getTo(link.href)}
                    onClick={handleLinkClick}
                    className={`inline-block px-0 text-white transition-all duration-200 hover:scale-110 max-sm:my-2 max-sm:mx-auto max-sm:text-center sm:max-md:mx-52 ${isActive
                        ? 'scale-110 border-b border-(--primary-color) text-(--primary-color)!'
                        : ''
                      }`}
                  >
                    {link.label}
